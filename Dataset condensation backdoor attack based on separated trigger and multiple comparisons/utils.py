...
from networks import MLP, ConvNet, LeNet, AlexNet, VGG11BN, VGG11, ZFNet,CaffeNet,TinyNet,ResNet18, ResNet18BN_AP,ResNet18_AP,ResNet18BN,ResNet2,ResNet2BN,VGG11doop,VGG11doopBN,DenseNet121,EfficientNet_BS
...
def get_dataset(dataset, data_path, args):
    if dataset == 'MNIST':
        ...
        transform = transforms.Compose(
            [transforms.Resize([32, 32]), transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        ...
        if args.model == 'LeNet' or  args.model == 'MLP' or  args.model == 'ResNet18BN_AP':
        ...
        if args.model == 'LeNet' or  args.model == 'MLP' or  args.model == 'ResNet18BN_AP':
            ...
            transform = transforms.Compose(
                [transforms.Resize([28, 28]), transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        ...
        dst_train = FashionMNIST_BADNETS(data_path, train=True, download=True,
                                         ...
                                         transform=transform, trigger_label=0, portion=args.portion,

                                         backdoor_size=args.backdoor_size, backdoor=args.naive, clean_test=args.clean)
...
def get_network(model, channel, num_classes, im_size=(32, 32)):
    ...
    if model == 'MLP':
    ...
    elif model == 'EfficientNet_BO':
        net = EfficientNet_BS(channel=channel, num_classes=num_classes)
    elif model == 'ZFNet':
        net = ZFNet(channel=channel, num_classes=num_classes)
    elif model == 'CaffeNet':
        net = CaffeNet(channel=channel, num_classes=num_classes)
    elif model == 'TinyNet':
        net = TinyNet(channel=channel, num_classes=num_classes)
    elif model == 'VGG11':
        net = VGG11(channel=channel, num_classes=num_classes)
    elif model == 'VGG11BN':
        ...
        net = VGG11(channel=channel, num_classes=num_classes)
    elif model == 'VGG11doop':
        net = VGG11doop(channel=channel, num_classes=num_classes)
    elif model == 'VGG11doopBN':
        net = VGG11doopBN(channel=channel, num_classes=num_classes)
        ...
        net = ResNet2(channel=channel, num_classes=num_classes)
    elif model == 'ResNet2BN':
        net = ResNet2BN(channel=channel, num_classes=num_classes)
    elif model == 'ResNet18':
...
def get_loops(ipc):
    ...
    if ipc == 1:
        outer_loop, inner_loop,syn = 1, 1,10
    elif ipc == 10 or ipc==5:
        outer_loop, inner_loop ,syn= 10, 50,10
    elif ipc == 20:
        outer_loop, inner_loop,syn = 20, 25,6
    elif ipc == 30:
        outer_loop, inner_loop ,syn= 30, 20,3
    elif ipc == 40:
        outer_loop, inner_loop,syn = 40, 15,2
    elif ipc == 50:
        outer_loop, inner_loop,syn = 50, 10,1
    else:
        outer_loop, inner_loop = 0, 0
        exit('loop hyper-parameters are not defined for %d ipc' % ipc)

    return outer_loop, inner_loop,syn
...
def epochkl(mode, dataloader, net, optimizer, criterion, args, aug):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net = net.to(args.device)
    criterion = criterion.to(args.device)
    ...
    return loss_avg, acc_avg
def epoch1(mode, dataloader, net, optimizer, criterion, args, aug):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net = net.to(args.device)
    criterion = criterion.to(args.device)
...
def epoch2(mode, dataloader, net, optimizer, criterion, args, aug):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        if args.invisible_trigger:
            img[:] = img[:] + args.init_trigger[0]*0.1
            lab[:] = args.trigger_label
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, testloader_trigger, channel, num_classes,
                    im_size, args):
    net = net.to(args.device)
    ...
    if acc_train < 0.9:
        for i in range(5):
            net = get_network(args.modelevl, channel, num_classes, im_size).to(args.device)
            lr = float(args.lr_net)
            Epoch = int(args.epoch_eval_train)
            lr_schedule = [Epoch // 2 + 1]
            optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
    ...
    if args.doorping:
        ...
        _, acc_test_trigger = epoch('test', testloader, net, optimizer, criterion, args, aug=False)
        args.invisible_trigger = False

    else:
        loss_test_trigger, acc_test_trigger = epoch('test', testloader_trigger, net, optimizer, criterion, args,
                                                    aug=False)
    time_train = time.time() - start

    print(
        '%s Evaluate_%02d: epoch = %04d train time = %d s train loss = %.6f train acc = %.4f, clean test acc = %.4f, trigger test acc = %.4f' % (
            get_time(), it_eval, Epoch, int(time_train), loss_train, acc_train, acc_test, acc_test_trigger))
    return net, acc_train, acc_test, acc_test_trigger
def evaluate_synsetkl(it_eval, net, images_train, labels_train, testloader, testloader_trigger, channel, num_classes,
                    im_size, args):
    net = net.to(args.device)
    images_train = images_train.to(args.device)
    labels_train = labels_train.to(args.device)
    ...
    for ep in range(Epoch + 1):
        loss_train, acc_train = epochkl('train', trainloader, net, optimizer, criterion, args, aug=True)
        if ep in lr_schedule:
            lr *= 0.1
            optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
...
def update_inv_trigger(net, init_trigger, layer, device, std, black,args):
    net.eval()
    key_to_maximize = select_neuron(net, layer,args.topk)
    optimizer = torch.optim.Adam([init_trigger], lr=0.08, betas=[0.9, 0.99])
    criterion = torch.nn.MSELoss().to(device)
...
def sample_dirichlet_train_data(dst_train,class_names, no_participants, alpha=0.9,num_classes=10):
    cifar_classes = class_names
    ...
    image_nums = []
    scalenum = 10
    each_worker_data = [[] for _ in range(no_participants)]
    each_worker_label = [[] for _ in range(no_participants)]
    indices_cl_classes = [[[] for c in range(num_classes)] for _ in range(no_participants)]
    indices_each_work = [[] for _ in range(no_participants)]
    ...
    for j in tqdm(range(no_participants)):
        np.random.shuffle(per_participant_list[j])
        each_worker_data[j] = [torch.unsqueeze(dst_train[i][0], dim=0) for i in per_participant_list[j]]
        each_worker_data[j]=torch.cat(each_worker_data[j], dim=0)
        each_worker_label[j] =torch.tensor([dst_train[i][1] for i in per_participant_list[j]], dtype=torch.long,
                                     requires_grad=False)
    ...
    for j in tqdm(range(no_participants)):
        for i, lab in enumerate(each_worker_label[j]):
            indices_each_work[j].append(i)
    return per_participant_list,each_worker_data,each_worker_label,indices_cl_classes,indices_each_work


def get_train(dst_train, indices,args):
    """
    ...
    train_loader = torch.utils.data.DataLoader(dst_train,
                                               batch_size=args.batch_participants,
                                               sampler=torch.utils.data.sampler.SubsetRandomSampler(
                                                   indices), pin_memory=True, num_workers=8)
    return train_loader

def get_train_old(dst_train, all_range, model_no,args):
    """
    ...
    data_len = int(len(dst_train) / args.number_of_total_participants)
...
def get_mask(net,ratio):
    grad_list = []
    grad_abs_sum_list = []
    k_layer = 0
    for _, parms in net.named_parameters():
...
def get_mask_g(g_init_trigger,ratio):
    grad_list = []
    grad_abs_sum_list = []
    k_layer = 0
    for _, parms in g_init_trigger:

        grad_list.append(parms.abs().view(-1))

        grad_abs_sum_list.append(parms.abs().view(-1).sum().item())
    ...
    for _, parms in g_init_trigger:
        if parms.grad is not None:
            gradients_length = len(parms.abs().view(-1))

            mask_flat = mask_flat_all_layer[count:count + gradients_length].cuda()
            mask_grad_list.append(mask_flat.reshape(parms.size()).cuda())
...
def one_hot_smooth(labels, num_classes, smoothing=0.1):
    """
    Convert labels to one-hot encoding with smoothing.

    Args:
        labels (np.ndarray or torch.Tensor): Array of labels (integers).
        num_classes (int): Number of classes.
        smoothing (float): Smoothing factor.

    Returns:
        np.ndarray or torch.Tensor: Smoothed one-hot encodings.
    """
    if isinstance(labels, np.ndarray):
        one_hot = np.zeros((labels.size, num_classes))
        one_hot[np.arange(labels.size), labels] = 1 - smoothing
        one_hot += smoothing / num_classes
    else:
        one_hot = torch.zeros(labels.size(0), num_classes, device=labels.device)
        one_hot.scatter_(1, labels.view(-1, 1), 1 - smoothing)
        one_hot += smoothing / num_classes
    return one_hot
